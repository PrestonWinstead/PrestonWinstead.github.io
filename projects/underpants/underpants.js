...
_.last = function(array, number) {
    ...
    if(!Array.isArray(array)) {
        ...
        for(let i = 0; i < number; i++){
            tempArray.unshift(array[number - i]);
        }
    }
    return tempArray;
}
...
_.each = function(collection, func) {
    if(Array.isArray(collection)) {
        ...
        for(var j in collection){
            func(collection[j], j, collection);
        }
    }
};
...
_.partition = function(array, func){
    var finalArray = [_.filter(array, func), _.reject(array, func)];
    return finalArray;
};
...
*/
//Goes through and manipulates each element in a collection, then returns a new array of modified values
...
//         _.each(arguments[i], function (b, c, d){
//             arguments[0][c] = arguments[i][c];
//         });
//     });
//     return arguments[0];
// };
